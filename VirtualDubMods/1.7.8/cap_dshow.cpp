...
//-----------------------------VBIAttach begin
#include <initguid.h>
DEFINE_GUID(CLSID_VBIAttach, 
0x34370657, 0xa1c4, 0x4a35, 0x9c, 0x4, 0x15, 0xaa, 0x30, 0x7d, 0x53, 0x54);
//-----------------------------VBIAttach end
...
//-----------------------------VBIAttach begin
	IPinPtr				mpShadowedRealVBIPin;		// the one on the cap filt
//-----------------------------VBIAttach end
...
//-----------------------------VBIAttach begin
	} else if (key.getInt("CapDShow: VBIAttach Enable", 0)) {
		hr = CoCreateInstance(CLSID_VBIAttach, 0, CLSCTX_INPROC_SERVER, IID_IBaseFilter, (void **)~mpCapTransformFilt);
		if (SUCCEEDED(hr)) {
			IPinPtr pPinTIn, pPinTVBIIn, pPinTOut, pPinCVBIOut;
			DS_VERIFY(mpGraphBuilder->AddFilter(mpCapTransformFilt, L"VBI attach"), "add VBI attach filter");
			DS_VERIFY(mpCapGraphBuilder2->FindPin(mpCapFilt, PINDIR_OUTPUT, &PIN_CATEGORY_VBI, NULL, TRUE, 0, ~pPinCVBIOut), "find capture VBI output");
			DS_VERIFY(mpCapGraphBuilder2->FindPin(mpCapTransformFilt, PINDIR_INPUT, NULL, NULL, TRUE, 0, ~pPinTIn), "find VBIAttach filter input");
			DS_VERIFY(mpCapGraphBuilder2->FindPin(mpCapTransformFilt, PINDIR_INPUT, NULL, NULL, TRUE, 1, ~pPinTVBIIn), "find VBIAttach filter VBI input");
			DS_VERIFY(mpCapGraphBuilder2->FindPin(mpCapTransformFilt, PINDIR_OUTPUT, NULL, NULL, TRUE, 0, ~pPinTOut), "find VBIAttach filter output");
			DS_VERIFY(mpGraphBuilder->Connect(mpRealCapturePin, pPinTIn), "connect capture -> VBIAttach");
			DS_VERIFY(mpGraphBuilder->Connect(pPinCVBIOut, pPinTVBIIn), "connect capture VBI -> VBIAttach VBI");
			mpShadowedRealCapturePin = mpRealCapturePin;
			mpShadowedRealVBIPin = pPinCVBIOut;
			mpRealCapturePin = pPinTOut;
		}
//-----------------------------VBIAttach end
	}
...
//-----------------------------VBIAttach begin
		if (mpShadowedRealVBIPin && VFW_E_NOT_CONNECTED == mpShadowedRealVBIPin->ConnectedTo(~pPinTest)) {
			// Reconnect transform filter input to VBI pin
			IPinPtr pPinTVBIIn;
			DS_VERIFY(mpCapGraphBuilder2->FindPin(mpCapTransformFilt, PINDIR_INPUT, NULL, NULL, TRUE, 1, ~pPinTVBIIn), "find VBIAttach VBI input");
			DS_VERIFY(mpGraphBuilder->Connect(mpShadowedRealVBIPin, pPinTVBIIn), "reconnect capture VBI -> VBIAttach VBI");
		}
//-----------------------------VBIAttach end
	}
